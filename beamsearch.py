'''
> 인용 : https://machinelearningmastery.com/beam-search-decoder-natural-language-processing/

## Beam Search Decoder

Beam Search는 Greedy Search 알고리즘(k=1)을 확장한 것이며, output sequences 의 리스트를
반환한다. beam search는 가능한 모든 단계(경로)를 탐색하고, k개를 유지한다. k는 사용자가 설정하는
hyper-parameter이며 beam의 수 또는 sequence 확률을 통한 병렬 탐색을 컨트롤 한다.
기계번역 작업에서는 보통 k=5 or k=10을 사용 한다. k가 크면 여러 candidate sequence들이
target sequence와 매칭될 가능성이 더 높아지기 때문에 좋은 성능을 보이나, 
이는 결과적으로는 decoding 속도를 감소시킨다.(성능 vs 속도 trade-off 관계)
...
'''
이제 출력으로 나온 sequence_index 를 단어 사전에서 찾아 나타내주면 decoding 완성
'''